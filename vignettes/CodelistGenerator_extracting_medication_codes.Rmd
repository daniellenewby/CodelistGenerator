---
title: "CodelistGenerator Extracting Medication Codes"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{CodelistGenerator_extracting_medication_codes}
...
## Creating a codelists for a single, multiple and classes of medications
For this vignette ee are going to create codelists for:
  * Single medication (metformin)
  * Multiple medications (metformin and insulin)
  * Class of medications (lipid lowering)
In these example we are going to generate a candidate codelists only looking for codes in the drug domain.
...
## Codelist from "Metformin" (ingredient) and its descendants
...
codes_from_descendants<-tbl(db, 
  sql(paste0("SELECT * FROM ",
     ...
     ".concept_ancestor"))) %>% 
  filter(ancestor_concept_id=="1503297") %>% 
...
toc()
# saveRDS(codes_from_descendants, here("Github","CodelistGenerator" , "vignettes","med_data_01.RData"))

```

```{r,  message=FALSE, warning=FALSE,echo=FALSE}
codes_from_descendants<-readRDS(here("Github","CodelistGenerator" , "vignettes","med_data_01.RData"))
```
...
First, let's do a simple search for a single keyword of "mteformin" without any exclusions, without searching synonyms, and including descendants but not ancestors.
```{r, eval=FALSE }
tic()
metformin_codes1<-get_candidate_codes(keywords="Metformin 1000 MG Oral Tablet",
                    domains="Drug",
...
tic()
diabetes_codes1<-get_candidate_codes(keywords="diabetes",
                    domains="Condition",
